...
pub async fn run_future(cache: SharedCache, client: reqwest::Client) {
    ...
    loop {
        ...
        tracing::info!(
            "bybit futures: spawning {} chunk workers ({} symbols)",
            num_chunks,
            instr.symbols.len()
        );
...
async fn run_chunk(
    ...
    loop {
        tracing::info!("bybit futures chunk-{}: connecting ({} symbols)...", chunk_id, symbols.len());
        ...
        match ws_result {
            Ok(Ok((ws, _))) => {
                attempt = 0;
                tracing::info!("bybit futures chunk-{}: connected", chunk_id);
                ...
                for chunk in symbols.chunks(10) {
                    ...
                    if let Err(e) = write.send(Message::Text(sub.to_string().into())).await {
                        tracing::error!("bybit futures chunk-{}: subscribe send error: {}", chunk_id, e);
                ...
                if !subscribe_failed {
                    for chunk in symbols.chunks(10) {
                        ...
                        if let Err(e) = write.send(Message::Text(sub.to_string().into())).await {
                            tracing::error!("bybit futures chunk-{}: orderbook subscribe send error: {}", chunk_id, e);
                            subscribe_failed = true;
                            break;
                        }
                        tokio::time::sleep(std::time::Duration::from_millis(100)).await;
                ...
                if subscribe_failed {
                    tracing::warn!("bybit futures chunk-{}: subscribe failed, reconnecting...", chunk_id);
                } else {
                    ...
                    tokio::select! {
                        msg = read.next() => {
                            ...
                            let msg = match msg {
                                Some(Ok(m)) => m,
                                Some(Err(e)) => {
                                    tracing::warn!("bybit futures chunk-{}: read error: {}", chunk_id, e);
                                    break;
                                }
                                None => {
                                    tracing::warn!("bybit futures chunk-{}: stream ended", chunk_id);
                                    break;
                                }
                            };
                            ...
                            let text = match msg {
                                ...
                                Message::Close(_) => {
                                    tracing::warn!("bybit futures chunk-{}: server closed connection", chunk_id);
                                    break;
                                }
                                _ => continue,
                            };
                            ...
                            let ws_msg: WsMsg = match serde_json::from_str(&text) {
                                Ok(v) => v,
                                Err(e) => {
                                    tracing::warn!("bybit futures chunk-{}: parse error: {}", chunk_id, e);
                                    continue;
                                }
                            };
                            ...
                            let ticker: TickerData = match serde_json::from_value(data_val) {
                                Ok(v) => v,
                                Err(e) => {
                                    tracing::warn!("bybit futures chunk-{}: parse ticker error: {}", chunk_id, e);
                                    continue;
                                }
                            };
                        ...
                        _ = ping_interval.tick() => {
                            ...
                            if let Err(e) = write.send(Message::Text(ping.to_string().into())).await {
                                tracing::error!("bybit futures chunk-{}: ping send error: {}", chunk_id, e);
                                break;
                            }
                        }
                    }
            ...
            Ok(Err(e)) => {
                tracing::error!("bybit futures chunk-{}: connection failed: {}", chunk_id, e);
            }
            Err(_) => {
                tracing::error!("bybit futures chunk-{}: connection timed out", chunk_id);
            }
        }
